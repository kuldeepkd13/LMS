...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      try {
        ...
        let endpoint = isSelected ? 'https://lms-osmi.onrender.com/user/getcourses' : 'https://lms-osmi.onrender.com/course/getAllCourse';
  ...
  const handleSubmit = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('https://lms-osmi.onrender.com/user/add-courses', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${token}`
