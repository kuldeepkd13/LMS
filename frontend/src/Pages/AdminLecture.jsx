...
const AdminLecture = () => {
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:4500/course/${courseId}/lectures`, {
          headers: {
  ...
  const handleCreateLecture = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:4500/lecture/create`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${token}`
  ...
  const handleDeleteLecture = async (lectureId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:4500/lecture/${lectureId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${token}`
  ...
  const handleUpdateCourse = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:4500/lecture/${updatingLecture._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${token}`
