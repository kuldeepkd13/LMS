...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    const fetchCourseDetails = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:4500/course/getAllCourse`, {
          headers: {
  ...
  const handleCreateCourse = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:4500/course/create`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${token}`
  ...
  const handleDeleteCourse = async (courseId) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:4500/course/${courseId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${token}`
  ...
  const handleUpdateCourse = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:4500/course/${updatingCourse._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          "Authorization": `${token}`
      ...
      {isCreating && (
        <div className="Create-form">
          <form onSubmit={handleCreateCourse}>
            <input type="text" name="name" value={newCourse.name} onChange={(e) => setNewCourse({ ...newCourse, name: e.target.value })} />
            <input type="text" name="description" value={newCourse.description} onChange={(e) => setNewCourse({ ...newCourse, description: e.target.value })} />
